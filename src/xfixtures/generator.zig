...
pub fn main() !void {
    ...
    // and a partial run would overwrite the direct fixture and then die
    // mid-way, leaving v3 segments `--force` does not know how to clean.
    // Stage C (D6) rewrites this for the `wal3-namespace` bundle kind and
    // removes this refusal.
    _ = &mainStageC; // keep the parked body analyzed so it cannot rot silently
    fatal("the fixture generator is STALE at the WAL v3 cutover (B2): " ++
        "the wal-v1-* fixtures cannot be regenerated by a v3 store; " ++
        "retired at Stage C — see src/xfixtures/generator.zig header", .{});
}
